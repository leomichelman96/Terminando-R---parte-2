...
iota <- na.omit(read_excel("C:/Econometria/iota.xlsx"))

iota <-  ts(iota$Fechar, start = 2017, frequency = 365)
...
acf(iota$,lend=2, lwd=5,col="darkblue",main= "Função Autocorrelação - FAC")              #Melhorando aspecto da FAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

pacf(iota$Fechar,lend=60, lwd=5,col="darkblue",main= "Função Autocorrelação Parcial - FACP")   #Melhorando aspecto da PAC
axis(1,tck = 1, col = "lightgrey", lty = "dotted")

#Teste ADF
ur.df(iota, "none", lags = 1)
